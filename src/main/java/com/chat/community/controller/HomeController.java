...
public class HomeController implements CommunityConstant {
    ...
    public String getIndexPage(Model model, Page page,
                               ...
                               @RequestParam(name = "Mode",defaultValue = "0") int Mode) {
    ...
    @RequestMapping(path = "/indexMode", method = RequestMethod.GET)
    public String getIndexModePage(Model model, Page page,
                               @RequestParam(name = "Mode") int Mode) {
        // 方法调用前,SpringMVC会自动实例化Model和Page,并将Page注入Model.
        // 所以,在thymeleaf中可以直接访问Page对象中的数据.
        page.setRows(discussPostService.findDiscussPostRowsByMode(Mode));
        page.setPath("/indexMode?Mode=" + Mode);

        List<DiscussPost> list = discussPostService.findDiscussPostsByMode(
